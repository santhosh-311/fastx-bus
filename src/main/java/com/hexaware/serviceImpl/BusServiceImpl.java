...
//	}
	...
	@Override
	public String deleteBus(Integer busId) throws BusNotFoundException, SeatNotFoundException {
		Bus bus=busRepo.findById(busId).orElseThrow(()-> new BusNotFoundException("Bus :"+busId+" Not Found"));
		if(LocalDate.now().compareTo(bus.getDate())<=0 && bus.getTotalSeats()==bus.getAvailableSeats()) {
			seatService.removeMultipleSeats(busId);
			busRepo.delete(bus);
		}
		else
