...
//	}
	...
	@Override
	public String removeMultipleSeats(Integer busId) throws BusNotFoundException, SeatNotFoundException{
		Bus bus = busRepo.findById(busId).orElseThrow(()-> new BusNotFoundException("Bus :"+busId+" Not Found"));
		List<Seat> seatList = bus.getSeat();
		Seat seat=null;
		for(Seat s:seatList){
			seat = seatRepo.findById(s.getSeatId()).orElseThrow(()-> new SeatNotFoundException("Seat Not Found"));
			if(seat!=null) {
				seatRepo.delete(seat);
			}
		}
